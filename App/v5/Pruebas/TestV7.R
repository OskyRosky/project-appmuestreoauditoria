...
body <- dashboardBody( 
  ...
  tabItem(tabName = "p2",
          ...
          uiOutput("variable_select_3"),
          br(),
          actionButton("update2", "Evaluar:"),
          br(),
          br(),
...
server <- function(input, output) {
  ...
  output$variable_select_2 <- renderUI({
    if (is.null(data2())) {
      ...
      selectInput("variable observada", "Elija una variable:", names(data2()))
    }
  })
  
  output$variable_select_3 <- renderUI({
    if (is.null(data2())) {
      return(NULL)
    } else {
      selectInput("variable auditada", "Elija una variable:", names(data2()))
    }
  })
